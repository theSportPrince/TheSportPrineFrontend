...
function MyChat({ users, fetchAgain, flag }) {
  ...
  const fetchChat = async () => {
    try {
      ...
      const filteredChat = response.data.find(
        (chat) => chat._id === "660fe234ee9e825c02d3a81d"
      );
      console.log("filtercchat", filteredChat,filteredChat._id);
   
      setChats(response.data);
      if (filteredChat) {
        setSelectedChat(filteredChat); // Set selectedChat to the filtered chat object
      } else {
        console.log("Chat not found with _id: 660fe234ee9e825c02d3a81d");
      }
